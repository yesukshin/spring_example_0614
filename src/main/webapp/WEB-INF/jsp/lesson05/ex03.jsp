...
<body>
   ...
   <fmt:formatNumber value="${number}" />
    ...
    통화원 : <fmt:formatNumber type="currency" value="${number}"/><br>
    달러 : <fmt:formatNumber type="currency" value="${number}" currencySymbol="$"/>
    ...
    3.141594(#.####)<fmt:formatNumber value="3.141594" pattern="#.####"  /><br>
    <hr>
    
    <h1>Date객체를 String으로 형태로 출력하기(fmt:formatDate)</h1>
    Date:  ${today} <br>
    <fmt:formatDate value="${today}" pattern="yyyy년 MM월 dd일 HH시 mm분 ss초" var="pattern1"/>
    pattern1 : ${pattern1} <br>
    <fmt:formatDate value="${today}" pattern="yyyy/MM/dd"  var="pattern2"/>
    pattern2 : ${pattern2} <br>
    
    <h1>String을 Date객체로 전환(fmt:parseDate)</h1>
    <fmt:parseDate value="${pattern2}" pattern="yyyy/MM/dd" />
</body>
</html>